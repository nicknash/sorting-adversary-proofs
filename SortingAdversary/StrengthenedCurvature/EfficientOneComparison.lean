import SortingAdversary.StrengthenedCurvature.StrengthenedVerification
import SortingAdversary.StrengthenedCurvature.StrengthenedEnvelope
...
/-- Per-informative-comparison potential budget for `K = 347/50`. -/
noncomputable def strengthenedComparisonBudget : ℝ :=
  Real.log (strengthenedK : ℝ) / (2 * Real.log 2)

theorem strengthenedComparisonBudget_nonneg : 0 ≤ strengthenedComparisonBudget := by
  exact div_nonneg (Real.log_nonneg (by norm_num [strengthenedK]))
...
private theorem historyPotential_child_le_of_logDet
    ...
    (hlog : Real.log (barrierHessian (o :: h) y).det -
        Real.log (barrierHessian h x).det ≤ Real.log (strengthenedK : ℝ)) :
    historyPotential (o :: h) - historyPotential h ≤ strengthenedComparisonBudget := by
  ...
  calc
    ...
    _ ≤ Real.log (strengthenedK : ℝ) / (2 * Real.log 2) :=
      (div_le_div_iff_of_pos_right (mul_pos (by norm_num) hlogtwo)).2 hlog
    _ = strengthenedComparisonBudget := rfl
...
private theorem near_center_step {n : ℕ} {h : History n} {x : Placement n}
  ...
  obtain ⟨i, hsegment⟩ := strengthened_schedule_cover hd0 hsmall
  obtain ⟨lambda, ha0, ha1, hb0, hb1, hcross, hl0, hl1, hcert⟩ :=
    strengthened_segment_certificate i hsegment
      motion.positiveEnergy_nonneg motion.positiveEnergy_le_one
  have htrials := min_trial_scaledLogDet_le_scalarEnvelope hc q
    (delta := delta) (a := scheduleA i delta) (b := scheduleB i delta)
    (lambda := (lambda : ℝ)) rfl ha0 ha1 hb0 hb1 hcross
      (Rat.cast_nonneg.mpr hl0) (by exact_mod_cast hl1)
  have hmin :
      min
        (scaledLogDet (augmentedRows h x o (delta + scheduleA i delta))
            (augmentedMotions h x o) (scheduleA i delta) -
          Real.log (barrierHessian h x).det)
        (scaledLogDet (augmentedRows h x o (scheduleB i delta - delta))
            (augmentedMotions h x o) (-scheduleB i delta) -
          Real.log (barrierHessian h x).det) <
        Real.log (strengthenedK : ℝ) := htrials.trans_lt hcert
  let plusValue := scaledLogDet
    (augmentedRows h x o (delta + scheduleA i delta))
    (augmentedMotions h x o) (scheduleA i delta) -
      Real.log (barrierHessian h x).det
  let minusValue := scaledLogDet
    (augmentedRows h x o (scheduleB i delta - delta))
    (augmentedMotions h x o) (-scheduleB i delta) -
      Real.log (barrierHessian h x).det
  by_cases hpm : plusValue ≤ minusValue
  · have hplus : plusValue ≤ Real.log (strengthenedK : ℝ) := by
    ...
    · rw [center_answerHistory]
      ...
      simpa [scaledLogDet, delta] using hplus
  · have hminusOrder : minusValue ≤ plusValue := le_of_not_ge hpm
    have hminus : minusValue ≤ Real.log (strengthenedK : ℝ) := by
    ...
    · rw [opposite_answerHistory]
      ...
      simpa [scaledLogDet, delta] using hminus
...
private theorem large_offset_quotient {n : ℕ} {h : History n}
    ...
    effectiveResistance h x (centerOrientation x q) /
      ...
      (strengthenedK : ℝ) - 1 := by
  let o := centerOrientation x q
  let delta := queryOffset h x o
  have hR := effectiveResistance_pos hx o
  have hsqrt := Real.sqrt_pos.2 hR
  have hdelta : 0 < delta := (by norm_num : (0 : ℝ) < 411 / 1000).trans hlarge
  ...
  rw [hratio]
  have hsq : (411 / 1000 : ℝ) ^ 2 ≤ delta ^ 2 := by nlinarith
  have hinv : 1 / delta ^ 2 ≤ 1 / (411 / 1000 : ℝ) ^ 2 :=
    one_div_le_one_div_of_le (by positivity) hsq
  calc
    1 / delta ^ 2 ≤ 1 / (411 / 1000 : ℝ) ^ 2 := hinv
    _ ≤ (strengthenedK : ℝ) - 1 := by norm_num [strengthenedK]
...
private theorem large_center_step {n : ℕ} {h : History n} {x : Placement n}
  ...
  have hslack : 0 < (BarrierRow.ofObservation o).slack x := by
    ...
    exact mul_pos (Real.sqrt_pos.2 (effectiveResistance_pos hc.1 o))
      ((by norm_num : (0 : ℝ) < 411 / 1000).trans hlarge)
  ...
  · rw [center_answerHistory]
    calc
      ...
      _ ≤ strengthenedComparisonBudget := by
        exact volumetricValue₂_cons_sub_le hc.1 o hslack
          (by norm_num [strengthenedK]) (large_offset_quotient hc.1 q hlarge)
...
noncomputable def efficientInformativeRule (n : ℕ) : InformativePotentialRule n where
  ...
  maxIncrease := strengthenedComparisonBudget
  maxIncrease_nonneg := strengthenedComparisonBudget_nonneg
  informativeStep := by
    intro h q hh _ _
    obtain ⟨x, hc⟩ := exists_volumetricCenter h hh
    by_cases hsmall : queryOffset h x (centerOrientation x q) ≤ (411 / 1000 : ℝ)
